---
title: State
summary: How to access and modify state during a request
icon: sliders-h
---
...
you can use the [`FluentState::withState()`](api:TractorCow\Fluent\State\FluentState::withState()) helper to safely perform this.
...
$nzRecords = FluentState::singleton()->withState(function (FluentState $state) {
    ...
    // Localised content queried in en_NZ locale
    return SiteTree::get();
});
```
