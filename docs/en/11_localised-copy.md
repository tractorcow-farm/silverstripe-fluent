...
summary: Different approaches to localising data, and how to use Localised Copy
icon: copy
---
...
We will apply [`FluentExtension`](api:TractorCow\Fluent\Extension\FluentExtension) to both classes. Note that this example is almost same in case your classes are versioned.
You need to apply [`FluentVersionedExtension`](api:TractorCow\Fluent\Extension\FluentVersionedExtension) instead of `FluentExtension` in such case.

The result of this setup is that the `Title` of both classes become localised.

#### Desired data state {#desired-data-state-1}
...
#### Desired data state {#desired-data-state-2}
...
This feature can be disabled globally via [`FluentExtension::withLocalisedCopyState()`](api:TractorCow\Fluent\Extension\FluentExtension::withLocalisedCopyState()). Example use:
